...
start_datetime = datetime.datetime(2022,5,29,20,00) # radartime
end_datetime = datetime.datetime(2022,5,29,20,1)
title_prefix = 'a)'
...
if var_to_plot == 'reflectivity':
    title_firstpart = r""+radarname+r" 0.5$^\circ$ "
    vmin, vmax = -30, 95
...
def convert_gr_table(grstr):
    ...
    return cmapdict
# @TaskGenerator    
def plot_radar_data(radar_file,terrain_data):
    ...
    # gl = ax2.gridlines(draw_labels=True, crs=proj)
    ...
    ax2.scatter(main_loc[1], main_loc[0], transform=ccrs.PlateCarree(), **main_loc_plot_opts) # Uncomment only for panel a) and supplemental
    ax2.set_aspect("auto")
    ...
    if timezone == mountain_tz:
        ...
        timezone_label = "ET"
    ax2.set_title(title_prefix+ " " + title_firstpart+"\n"+
                r""+curr_local_time.strftime("%Y/%m/%d %H:%M")+timezone_label, size=10)
    ...
    display.plot_point(radar.longitude['data'][0], radar.latitude['data'][0], color = "black", markersize = 5)
...
for radfilenum, filename in enumerate((radfiles)):
    plot_radar_data(filename, terrain_data=terrain_data)