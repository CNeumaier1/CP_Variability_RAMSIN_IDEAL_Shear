...
def plot_radar_data(radar_file,terrain_data,i,fig):
   ...
   #ax2 = fig.add_subplot(2, 2, i, projection=proj)
    ...
    # ax2.text(.01, 1.1, title_prefix, ha='left', va='top', weight = 'bold', size = 12, transform=ax2.transAxes)
...
# plotting options
figuresize=[4.25, 4.25]  ##width, height
fig = plt.figure(figsize = figuresize)
# fig.add_gridspec(2, 2, height_ratios=[1, 1]) 
for i in range(4):
    if i == 0:
        radarname = 'KCYS'
        cent_lat = 40
    ...
    if var_to_plot == 'reflectivity':
        # title_firstpart = r""+title_prefix+radarname+r" Reflectivity 0.5$^\circ$ "
        title_firstpart = r""+title_prefix+" "+radarname+r" 0.5$^\circ$ "
        vmin, vmax = -30, 95
...
# cax_reflectivity = fig.add_axes([1, 0.4, 0.01, 0.5])
cax_reflectivity = fig.add_axes([1.02, 0.2, 0.02, 0.6])
...
cbar_reflectivity.ax.tick_params(labelsize=10) 
