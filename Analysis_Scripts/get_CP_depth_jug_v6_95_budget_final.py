...
def calc_cp_prime(outfilename, cp2_start, thv_mean):
    ...
    #### need to get thv_prime_1, thv_prime_2, thv_bar1, thv_bar2 into arrays and into file
...
def cp_depth(outfilename,thv_prime_mask,nt,thv_threshold,cp2_start,alt_list,q,cp_mode = 1):
    ...
    for t in time_range:
            ...
            # Create a dictionary to store the maximum z index for each unique y point
            max_z_for_y = {}
            for y, z in zip(y_points, z_points):
                if y in max_z_for_y:
                    ...
                    max_z_for_y[y] = z
            zy_list.append(list(max_z_for_y.items()))
        ...
        ### for rkw analysis, need to also return max_z_for_y
    return max_depth_list, zy_list
...
def calc_depth(filePath,wind_prof,CP1_T, CP2_T, time_d,alt_list, thv, q):
    ...
    max_depth_list_cp1, depth_list_cp1 = cp_depth(outfilename2,thv_prime_mask_cp1,nt,thv_threshold1,cp2_start,alt_list,q,cp_mode = 1)
    max_depth_list_cp2, depth_list_cp2 = cp_depth(outfilename2,thv_prime_mask_cp2,nt,thv_threshold2,cp2_start,alt_list,q,cp_mode = 2)

    
    return max_depth_list_cp1, max_depth_list_cp2, depth_list_cp1, depth_list_cp2, thv_bar1,thv_bar2, thv_prime_cp1, thv_prime_cp2, u_bar1,u_bar2,v_bar1,v_bar2
...
def make_cpdepth_array(filePaths,alt_list, thv, q):
    ...
    depth_array_all = np.zeros((len(wind_list),len(temp_list),len(temp_list),len(time_list),2,50), dtype = 'object')
    thv_array = np.zeros((len(wind_list),len(temp_list),len(temp_list),len(time_list),2,50,99))
    thv_prime_array = np.zeros((len(wind_list),len(temp_list),len(temp_list),len(time_list),2,50,99,2998))
                ...
                for k,CP2_temp in enumerate(temp_list):
                    for t,time in enumerate(time_list):
                        if f"CP1_{CP1_temp}_CP2_{CP2_temp}_{time:03}_{wind}" in filePath:
                            ...
                            cp1_depth_list, cp2_depth_list, depth_list_cp1, depth_list_cp2,thv_bar1,thv_bar2,thv_prime_cp1, thv_prime_cp2, u_bar1,u_bar2,v_bar1,v_bar2 = calc_depth(filePath,wind,CP1_temp,CP2_temp,time,alt_list, thv, q)
                            ...
                            depth_array_all[w,c,k,t,0,:len(depth_list_cp1)] = depth_list_cp1
                            depth_array_all[w,c,k,t,1,:len(depth_list_cp2)] = depth_list_cp2
                            
                            thv_array[w,c,k,t,0,:len(thv_bar1),:] = thv_bar1
                            thv_array[w,c,k,t,1,:len(thv_bar2),:] = thv_bar2

                            thv_prime_array[w,c,k,t,0,:len(thv_bar1),:,:] = thv_prime_cp1
                            thv_prime_array[w,c,k,t,1,:len(thv_bar2),:,:] = thv_prime_cp2
    ...
    return depths_dataframe, test_array, depth_array_all, thv_array,thv_prime_array, u_array, v_array


def make_env_file(env_savePath, thv_array, thv_prime_array,u_array, v_array):
    ...
    test_dataset.create_dataset('thv_prime', data=thv_prime_array)
...
def make_cpdepth_file(filePaths, savePath,alt_list,thv, q, mode =1):
    ...
    if mode ==1:
        ...
        depths_dataframe, depths_array,all_depths_array, thv_array, thv_prime_array, u_array, v_array = make_cpdepth_array(filePaths,alt_list, thv, q)
        env_savePath = "./envir_cps_rkw_va1.h5"
        make_env_file(env_savePath, thv_array, thv_prime_array, u_array, v_array)
        return 
    
    test_dataset = h5py.File(savePath+".h5", 'w')
    test_dataset.create_dataset('all_depths', data=all_depths_array)
    test_dataset.close()
...
for thv in thv_thresholds:
    for q in quantile_test:
        ...
        savePath = f"./cpdepths_all_rkw/thv{round(thv,2):03}_q{q}"
        if (thv ==0.4) and (q==92):
            mode =1
        else:
            mode=1
        make_cpdepth_file(filePaths,savePath,alt, thv, q, mode = 3)